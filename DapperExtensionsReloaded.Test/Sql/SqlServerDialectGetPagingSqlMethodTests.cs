...
namespace DapperExtensionsReloaded.Test.Sql
{
    public class SqlServerDialectGetPagingSqlMethodTests : SqlServerDialectFixtureBase
    {
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Dialect.GetPagingSql(null, 0, 10, 10, new Dictionary<string, object>()));
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Dialect.GetPagingSql(string.Empty, 0, 10, 10, new Dictionary<string, object>()));
            Assert.Equal("sql", ex.ParamName);
        }
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Dialect.GetPagingSql("SELECT [schema].[column] FROM [schema].[table]", 0, 10, 10, null));
        ...
        public void Select_ReturnsSql()
        {
            ...
            var result = Dialect.GetPagingSql("SELECT [column] FROM [schema].[table]", 0, 10, 10, parameters);
        ...
        public void SelectDistinct_ReturnsSql()
        {
            ...
            var result = Dialect.GetPagingSql("SELECT DISTINCT [column] FROM [schema].[table]", 0, 10, 10, parameters);
            Assert.Equal(sql, result);
            Assert.Single(parameters);
            Assert.Equal(1, parameters["@_pageStartRow"]);
        }
        ...
        public void SelectOrderBy_ReturnsSql()
        {
            ...
            var result = Dialect.GetPagingSql("SELECT [column] FROM [schema].[table] ORDER BY [column] DESC", 0, 10, 10, parameters);
            Assert.Equal(sql, result);
            Assert.Single(parameters);
            Assert.Equal(1, parameters["@_pageStartRow"]);
        }
