...
namespace DapperExtensions.Predicates
{
    public static class Predicates
    {
        ...
        public static IFieldPredicate Field<T>(Expression<Func<T, object>> expression, Operator op, object value, bool not = false) where T : class
        {
            ...
            {
                throw new ArgumentException("Could not retrieve property info from lambda expression.", nameof(expression));
            }

            return new FieldPredicate<T>(GetGenerator)
            {
            ...
            where T2 : class
        {
            ...
            {
                throw new ArgumentException("Could not retrieve property info from lambda expression.", nameof(expression2));
            }

            return new PropertyPredicate<T, T2>(GetGenerator)
            {
                PropertyName = propertyInfo.Name,
        ...
        public static IExistsPredicate Exists<TSub>(IPredicate predicate, bool not = false)
            where TSub : class
        {
            return new ExistsPredicate<TSub>(GetGenerator)
            {
                Not = not,
                Predicate = predicate
            };
            ...
            where T : class
        {
            ...
            return new BetweenPredicate<T>(GetGenerator)
            {
                Not = not,
                PropertyName = propertyInfo.Name,
                Value = values
        ...
        private static ISqlGenerator GetGenerator()
        {
            return DapperExtensions.Instance.SqlGenerator;
        }
    }
}